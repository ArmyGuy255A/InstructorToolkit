Function Show-ClassManagerNewClass ( $classInfo ) {
    Clear-Host
...
Function Show-ClassManagerSelectClass ( $classInfo ) {
    Clear-Host
...
Function Show-ClassManagerRemoveClass ( $classInfo ) {
    Clear-Host
    $menuChoices = $classInfo.AvailableClasses
...
Function Show-ClassManagerMenu () {
    while (-1) {
        $menuChoices = @(
        "Select Class",
        "Add New Class",
        ...
        $activeClassInfo
        $result = Show-STMenu -Title "Class Manager Menu" -Choices $menuChoices -Back -Info @("Active Class", $activeClassInfo.ActiveClass)

        switch ($result) {
            1 { Show-ClassManagerSelectClass $activeClassInfo }
            2 { Show-ClassManagerNewClass $activeClassInfo}
            3 { Show-ClassManagerRemoveClass $activeClassInfo }
            "B" {return}
            Default {}
        }
    }
